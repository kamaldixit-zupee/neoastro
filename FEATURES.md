...
| Astrologer profile | ✅ | [Features/Home/AstrologerProfileView.swift](NeoAstro/Features/Home/AstrologerProfileView.swift) | `screens/astrologerProfile/index.tsx` | Now backed by `AstrologerProfileViewModel`; stories carousel, education, reviews, notify-me, presence-driven status pill / wait-time chip all live |
| Astrologer stories carousel | ✅ | [Features/Home/AstrologerStoriesView.swift](NeoAstro/Features/Home/AstrologerStoriesView.swift) | `AllStories.tsx` | Horizontal carousel of 78 pt circular thumbs with gold ring; tap → full-screen Instagram-style viewer with progress bars, tap-left/right zones, hold-to-pause, image + video |
| Astrologer reviews | ✅ | [Features/Home/AstrologerProfileView.swift](NeoAstro/Features/Home/AstrologerProfileView.swift) | `/v1.0/astrologer/reviews` | Top 5 reviews surface inline (avatar + 5-star row + comment + date); average rating folds into the rating row |
| Astrologer popup details | 🟡 | [Services/AstrologerService.swift](NeoAstro/Services/AstrologerService.swift) | `getPopupDetails` | Service + DTOs wired; loaded into `AstrologerProfileViewModel.popup`; UI surface deferred |
| Astrologer metadata | 🟡 | [Services/AstrologerService.swift](NeoAstro/Services/AstrologerService.swift) | `getAstrologerMetadata` | Service + DTOs wired; loaded into `AstrologerProfileViewModel.metadata`; UI surface deferred |
| "Notify me when online" | ✅ | [Features/Home/AstrologerProfileView.swift](NeoAstro/Features/Home/AstrologerProfileView.swift) | `chat/notifyUser` | Glass card shown only when offline; states: idle / requesting / subscribed / failed |
| Best astrologers modal | ⏳ | — | `bestAstrologers/index.tsx` | |
| Live players info | ⏳ | — | `getLivePlayersInfo` | |
| Real-time online indicator | ✅ | [Features/Home/AstrologerProfileView.swift](NeoAstro/Features/Home/AstrologerProfileView.swift) | `ASTROLOGER_STATUS_UPDATE` socket | Profile status pill reads `RealtimeStore.presence[astroId]?.chatStatus` first, falls back to list-payload `status.state` |
| Wait-time chip per astrologer | ✅ | [Features/Home/AstrologerProfileView.swift](NeoAstro/Features/Home/AstrologerProfileView.swift) | `ASTROLOGER_WAITTIME_UPDATE` | Orange "Wait ~N min" pill rendered when offline + `presence.waitTime > 0` |
...
| Read receipts | ✅ | [Features/Chat/ChatViewModel.swift](NeoAstro/Features/Chat/ChatViewModel.swift) | `HUMAN_ANSWER_SEEN` | High-water-mark `lastSeenSequenceId` debounce; emitted only when an astrologer message scrolls into view via `MessageBubble.onAppear` |
| Recording indicators (astrologer) | ✅ | [Features/Chat/ChatView.swift](NeoAstro/Features/Chat/ChatView.swift) | `ASTRO_RECORDING_*` | `RealtimeStore.astrologerRecording` flips on/off; status header shows red "Recording…" with waveform glyph |
...
| Recharge CTA from chat | ✅ | [Features/Chat/MessageBubble.swift](NeoAstro/Features/Chat/MessageBubble.swift) | `IN_CHAT_RECHARGE_CTA_CLICKED` | Orange-tinted glass card on `SYSTEM_LOW_BALANCE` bubbles; tap emits the analytics event then routes to Wallet via `DeepLinkRouter.intent = .wallet` |
...
| Chat history list | ✅ | [Features/Conversations/ConversationsView.swift](NeoAstro/Features/Conversations/ConversationsView.swift) | `screens/conversations/index.tsx` | Glass list with avatar + last-message preview + unread pill + active/ended badge; reachable via bubble icon in HomeView toolbar |
| Per-astrologer chat history | ✅ | [Features/Conversations/ChatHistoryView.swift](NeoAstro/Features/Conversations/ChatHistoryView.swift) | `chat/getHistoryWithAstrologer` | Read-only viewer reusing `MessageBubble` for text/audio/image variants; resume CTA emits `INITIATE_CHAT` for active sessions |
| Live chat details fetch | ⏳ | — | `chat/getLiveChatDetails` | Endpoint covered by realtime `CHAT_STARTED`; fold-in if a separate REST snapshot is needed later |
| Delete chat with one astrologer | ✅ | [Features/Conversations/ConversationsViewModel.swift](NeoAstro/Features/Conversations/ConversationsViewModel.swift) | `chat/deleteChatHistory` | Swipe-to-delete on each row |
| Delete all chat history | ✅ | [Features/Conversations/ConversationsViewModel.swift](NeoAstro/Features/Conversations/ConversationsViewModel.swift) | `chat/deleteAllChatHistory` | Trash button + confirmation dialog |
...
| Astrologer-online system notification | ✅ | [Features/Home/HomeView.swift](NeoAstro/Features/Home/HomeView.swift) | `ASTROLOGER_ONLINE_NOTIFICATION` | Glass banner with green-tinted backdrop renders at the top of Home; tap routes to that astrologer's profile (when present in the loaded list) and clears the banner |
...
- **Batch 8 — Chat polish + presence UI.** [RealtimeStore](NeoAstro/Realtime/RealtimeStore.swift) gained `astrologerRecording: Bool` (set by `ASTRO_RECORDING_STARTED`, cleared by `ASTRO_RECORDING_STOPPED` and on chat-end); dispatch table extended. [ChatEventHandler](NeoAstro/Realtime/handlers/ChatEventHandler.swift) handles the two new events. [ChatViewModel](NeoAstro/Features/Chat/ChatViewModel.swift) gained `messageBecameVisible(_:)` + `lastSeenSequenceId` high-water-mark to debounce `HUMAN_ANSWER_SEEN` emits. [ChatView](NeoAstro/Features/Chat/ChatView.swift) wires `MessageBubble.onAppear` into the read-receipt path and adds a red "Recording…" state to the status header (priority above typing). [MessageBubble](NeoAstro/Features/Chat/MessageBubble.swift) gained an `onRecharge` callback and a dedicated orange-tinted glass card for `SYSTEM_LOW_BALANCE` messages with a "Recharge" button; ChatView's `handleRechargeFromChat()` emits `IN_CHAT_RECHARGE_CTA_CLICKED` and routes to Wallet via `DeepLinkRouter.intent = .wallet`. [SocketManager](NeoAstro/Realtime/SocketManager.swift) gained an `EmptyPayload` marker + no-payload `emit(_:)` overload. [HomeView](NeoAstro/Features/Home/HomeView.swift) renders an `astrologerOnlineBanner(_:)` glass card at the top of the feed when `RealtimeStore.astrologerOnlineBanner` is set; tap → push that astrologer's profile (if loaded) + clear the banner. [Models/API/AstrologerAPI.swift](NeoAstro/Models/API/AstrologerAPI.swift) gained `AstrologerPopupResponse` / `AstrologerPopupContent` and `AstrologerMetadataResponse` / `AstrologerMetadata`. [AstrologerService](NeoAstro/Services/AstrologerService.swift) gained `popupDetails(astroId:)` and `metadata(astroId:)`; [AstrologerProfileViewModel](NeoAstro/Features/Home/AstrologerProfileViewModel.swift) loads them in parallel with profile + reviews (UI surface for popup + metadata is deferred).
- **Batch 7 — Conversations list + per-astrologer chat history.** New [Models/API/ChatHistoryAPI.swift](NeoAstro/Models/API/ChatHistoryAPI.swift) with `ConversationSummary` (resolved across both `chats` and `conversations` server-side keys), `HistoricalMessage` (mixed user / astrologer with `fromUser` resolver), delete/list bodies. New [Services/ChatHistoryService.swift](NeoAstro/Services/ChatHistoryService.swift) — `conversations(skip:limit:)`, `messages(with:skip:limit:)`, `deleteHistory(with:)`, `deleteAllHistory()`. New [ConversationsViewModel](NeoAstro/Features/Conversations/ConversationsViewModel.swift) and [ConversationsView](NeoAstro/Features/Conversations/ConversationsView.swift) — glass list with avatar + active green dot, last-message preview (📷 / 🎤 prefixes for media), unread pink-pill, swipe-to-delete, trash button + confirmation for clear-all. New [ChatHistoryViewModel](NeoAstro/Features/Conversations/ChatHistoryView.swift) + [ChatHistoryView](NeoAstro/Features/Conversations/ChatHistoryView.swift) — read-only conversation viewer reusing `MessageBubble` for text/audio/image variants; active sessions get a "Continue chat" CTA that re-emits `INITIATE_CHAT` (with `continueSession: true`) and waits for `CHAT_STARTED` to land in `RealtimeStore.activeChat`; ended sessions get an archived footer. [HomeView](NeoAstro/Features/Home/HomeView.swift) toolbar gained a bubble icon entry that pushes Conversations.
- **Batch 6 — Astrologer profile completion.** [Models/API/AstrologerAPI.swift](NeoAstro/Models/API/AstrologerAPI.swift) gained `AstrologerProfileResponse` / `AstrologerProfileData` / `AstrologerProfileDetail` (full-detail wrapper), `AstrologerStory` (image + video + caption + thumbnail with `mediaURL` / `thumbURL` / `isVideo` helpers), `AstrologerEducation`, `AstrologerReview` + `AstrologerReviewsResponse`, `NotifyUserBody`. [Services/AstrologerService.swift](NeoAstro/Services/AstrologerService.swift) gained `getProfile(astroId:)`, `reviews(astroId:)`, `notifyMeWhenOnline(astroId:)`. New [AstrologerProfileViewModel](NeoAstro/Features/Home/AstrologerProfileViewModel.swift) (`@Observable @MainActor`) loads profile + reviews in parallel and tracks the notify-me state machine (`idle → requesting → subscribed | failed(_)`). [AstrologerProfileView](NeoAstro/Features/Home/AstrologerProfileView.swift) extended significantly: presence-driven status pill (online green or "Wait ~N min" orange) sourced from `RealtimeStore.presence`, stories carousel (78 pt gold-ringed thumbs with play overlay for videos) opening a full-screen viewer, new education section, notify-me card shown only when offline, reviews section showing the top 5 with 5-star row + comment + date. New [AstrologerStoriesView](NeoAstro/Features/Home/AstrologerStoriesView.swift) — Instagram-style full-screen viewer with paged TabView, top progress bars, tap-left/right zones, long-press-to-pause, image + video (via `AVKit.VideoPlayer`), 5 s auto-advance.
